...
fn generic_set(
    ...
    let res = db.get_or_create(dbindex, &key).set(val);
...
fn append(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    let r = {
        let mut oldval = db.get_or_create(dbindex, &key);
...
fn get(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 2);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    generic_get(db, dbindex, key, true)
}

fn mget(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate!(parser.argv.len() >= 2, "Wrong number of parameters");
...
fn getrange(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 4);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
...
fn getbit(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 3);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
...
fn strlen(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 2);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let obj = db.get(dbindex, &key);
...
fn pfcount(parser: &ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_gte!(parser, 2);
...
fn pfmerge(parser: &ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    {
        let mut value = db.get_or_create(dbindex, &key);
        *value = val;
    }
    db.key_updated(dbindex, &key);
    r
...
fn generic_rpoplpush(
    ...
    let el = {
        let mut sourcelist = match db.get_mut(dbindex, source) {
            Some(sourcelist) => {
    ...
    let resp = {
        let mut destinationlist = db.get_or_create(dbindex, destination);
...
fn lindex(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 3);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let index = try_validate!(parser.get_i64(2), "Invalid index");
...
fn llen(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 2);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
...
fn lrange(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 4);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let start = try_validate!(parser.get_i64(2), "Invalid range");
    let stop = try_validate!(parser.get_i64(3), "Invalid range");
...
fn lrem(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    let value = try_validate!(parser.get_vec(3), "Invalid value");
    let r = match db.get_mut(dbindex, &key) {
        Some(mut el) => match el.lrem(count < 0, count.abs() as usize, value) {
...
fn lset(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    let value = try_validate!(parser.get_vec(3), "Invalid value");
    let r = match db.get_mut(dbindex, &key) {
        Some(mut el) => match el.lset(index, value) {
            Ok(()) => Response::Status("OK".to_owned()),
            Err(err) => Response::Error(err.to_string()),
        },
...
fn ltrim(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    let r = match db.get_mut(dbindex, &key) {
        Some(mut el) => match el.ltrim(start, stop) {
            Ok(()) => Response::Status("OK".to_owned()),
            Err(err) => Response::Error(err.to_string()),
        },
...
fn sadd(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    {
        let mut el = db.get_or_create(dbindex, &key);
        for i in 2..parser.argv.len() {
            let val = try_validate!(parser.get_vec(i), "Invalid value");
...
fn sismember(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 3);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
...
fn srandmember(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_gte!(parser, 2);
...
fn smembers(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 2);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let value = match db.get(dbindex, &key) {
        Some(el) => el,
...
fn smove(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    {
        let mut source = match db.get_mut(dbindex, &source_key) {
            Some(s) => s,
            None => return Response::Integer(0),
        };
    ...
    let set_max_intset_entries = db.config.set_max_intset_entries;
    {
        let mut destination = db.get_or_create(dbindex, &destination_key);
...
fn scard(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    validate_arguments_exact!(parser, 2);
    let key = try_validate!(parser.get_vec(1), "Invalid key");
...
fn sdiff(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let el = match db.get(dbindex, &key) {
        Some(e) => e,
        None => return Response::Array(vec![]),
    };
    let sets = get_values!(2, parser.argv.len(), parser, db, dbindex, Value::Nil);
...
fn sdiffstore(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    let set = {
        ...
        let sets = get_values!(3, parser.argv.len(), parser, db, dbindex, Value::Nil);
        match el.sdiff(&sets) {
            Ok(set) => set,
            Err(err) => return Response::Error(err.to_string()),
        }
...
fn sinter(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let el = match db.get(dbindex, &key) {
        Some(e) => e,
        None => return Response::Array(vec![]),
...
fn sinterstore(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    ...
    let set = {
        let key = try_validate!(parser.get_vec(2), "Invalid key");
        let el = db.get(dbindex, &key).unwrap_or_default();
        let sets = get_values!(3, parser.argv.len(), parser, db, dbindex, Value::Nil);
        match el.sinter(&sets) {
            Ok(set) => set,
            Err(err) => return Response::Error(err.to_string()),
...
fn sunion(parser: &mut ParsedCommand, db: &mut Database, dbindex: usize) -> Response {
    let key = try_validate!(parser.get_vec(1), "Invalid key");
    let el = db.get(dbindex, &key).unwrap_or_default();
    let sets = get_values!(2, parser.argv.len(), parser, db, dbindex, Value::Nil);
...
fn zinter_union_store(
    ...
    let value = {
        ...
        let zsets = get_values!(3, 3 + numkeys, parser, db, dbindex, Value::Nil);
...
fn info(parser: &mut ParsedCommand, db: &mut Database) -> Response {
...
mod command_names {
    ...
    pub const SETNX: &str = "setnx";
    pub const SETEX: &str = "setex";
    pub const PSETEX: &str = "psetex";
    pub const APPEND: &str = "append";
    pub const STRLEN: &str = "strlen";
    pub const DEL: &str = "del";
    pub const EXISTS: &str = "exists";
    pub const SETBIT: &str = "setbit";
    pub const GETBIT: &str = "getbit";
    pub const SETRANGE: &str = "setrange";
    pub const GETRANGE: &str = "getrange";
    ...
    pub const RPUSHX: &str = "rpushx";
    pub const LPUSHX: &str = "lpushx";
    pub const LINSERT: &str = "linsert";
    pub const RPOP: &str = "rpop";
    pub const LPOP: &str = "lpop";
    pub const RPOPLPUSH: &str = "rpoplpush";
    pub const BRPOP: &str = "brpop";
    pub const BLPOP: &str = "blpop";
    pub const BRPOPLPUSH: &str = "brpoplpush";
    pub const LLEN: &str = "llen";
    pub const LINDEX: &str = "lindex";
    pub const LSET: &str = "lset";
    pub const LRANGE: &str = "lrange";
    pub const LTRIM: &str = "ltrim";
    pub const LREM: &str = "lrem";
    pub const SADD: &str = "sadd";
    pub const SREM: &str = "srem";
    pub const SMOVE: &str = "smove";
    pub const SISMEMBER: &str = "sismember";
    pub const SCARD: &str = "scard";
    pub const SPOP: &str = "spop";
    pub const SRANDMEMBER: &str = "srandmember";
    pub const SINTER: &str = "sinter";
    pub const SINTERSTORE: &str = "sinterstore";
    pub const SUNION: &str = "sunion";
    pub const SUNIONSTORE: &str = "sunionstore";
    pub const SDIFF: &str = "sdiff";
    pub const SDIFFSTORE: &str = "sdiffstore";
    pub const SMEMBERS: &str = "smembers";
    pub const SSCAN: &str = "sscan";
    pub const ZADD: &str = "zadd";
    pub const ZINCRBY: &str = "zincrby";
    pub const ZREM: &str = "zrem";
    pub const ZREMRANGEBYSCORE: &str = "zremrangebyscore";
    pub const ZREMRANGEBYRANK: &str = "zremrangebyrank";
    pub const ZREMRANGEBYLEX: &str = "zremrangebylex";
    pub const ZUNIONSTORE: &str = "zunionstore";
    pub const ZINTERSTORE: &str = "zinterstore";
    pub const ZRANGE: &str = "zrange";
    pub const ZREVRANGE: &str = "zrevrange";
    pub const ZRANGEBYSCORE: &str = "zrangebyscore";
    pub const ZREVRANGEBYSCORE: &str = "zrevrangebyscore";
    pub const ZRANGEBYLEX: &str = "zrangebylex";
    pub const ZREVRANGEBYLEX: &str = "zrevrangebylex";
    pub const ZCOUNT: &str = "zcount";
    pub const ZLEXCOUNT: &str = "zlexcount";
    pub const ZCARD: &str = "zcard";
    pub const ZSCORE: &str = "zscore";
    pub const ZRANK: &str = "zrank";
    pub const ZREVRANK: &str = "zrevrank";
    pub const ZSCAN: &str = "zscan";
    pub const HSET: &str = "hset";
    pub const HSETNX: &str = "hsetnx";
    pub const HGET: &str = "hget";
    pub const HMSET: &str = "hmset";
    pub const HMGET: &str = "hmget";
    pub const HINCRBY: &str = "hincrby";
    pub const HINCRBYFLOAT: &str = "hincrbyfloat";
    pub const HDEL: &str = "hdel";
    pub const HLEN: &str = "hlen";
    pub const HSTRLEN: &str = "hstrlen";
    pub const HKEYS: &str = "hkeys";
    pub const HVALS: &str = "hvals";
    pub const HGETALL: &str = "hgetall";
    pub const HEXISTS: &str = "hexists";
    pub const HSCAN: &str = "hscan";
    pub const INCRBY: &str = "incrby";
    pub const DECRBY: &str = "decrby";
    pub const INCRBYFLOAT: &str = "incrbyfloat";
    pub const GETSET: &str = "getset";
    pub const MSET: &str = "mset";
    pub const MSETNX: &str = "msetnx";
    pub const RANDOMKEY: &str = "randomkey";
    pub const SELECT: &str = "select";
    pub const MOVE: &str = "move";
    pub const RENAME: &str = "rename";
    pub const RENAMENX: &str = "renamenx";
    pub const EXPIRE: &str = "expire";
    pub const EXPIREAT: &str = "expireat";
    pub const PEXPIRE: &str = "pexpire";
    pub const PEXPIREAT: &str = "pexpireat";
    pub const KEYS: &str = "keys";
    pub const SCAN: &str = "scan";
    pub const DBSIZE: &str = "dbsize";
    pub const AUTH: &str = "auth";
    pub const PING: &str = "ping";
    pub const ECHO: &str = "echo";
    pub const SAVE: &str = "save";
    pub const BGSAVE: &str = "bgsave";
    pub const BGREWRITEAOF: &str = "bgrewriteaof";
    pub const SHUTDOWN: &str = "shutdown";
    pub const LASTSAVE: &str = "lastsave";
    ...
    pub const FLUSHDB: &str = "flushdb";
    pub const FLUSHALL: &str = "flushall";
    pub const SORT: &str = "sort";
    pub const INFO: &str = "info";
    pub const MONITOR: &str = "monitor";
    pub const TTL: &str = "ttl";
    pub const PTTL: &str = "pttl";
    pub const PERSIST: &str = "persist";
    pub const SLAVEOF: &str = "slaveof";
    ...
    pub const RESTORE_ASKING: &str = "restore-asking";
    ...
    pub const SLOWLOG: &str = "slowlog";
    pub const SCRIPT: &str = "script";
    pub const TIME: &str = "time";
    pub const BITOP: &str = "bitop";
    pub const BITCOUNT: &str = "bitcount";
    pub const BITPOS: &str = "bitpos";
    pub const WAIT: &str = "wait";
    pub const COMMAND: &str = "command";
    pub const GEOADD: &str = "geoadd";
    pub const GEORADIUS: &str = "georadius";
    pub const GEORADIUSBYMEMBER: &str = "georadiusbymember";
    pub const GEOHASH: &str = "geohash";
    pub const GEOPOS: &str = "geopos";
    pub const GEODIST: &str = "geodist";
    pub const PFSELFTEST: &str = "pfselftest";
    pub const PFADD: &str = "pfadd";
    pub const PFCOUNT: &str = "pfcount";
    pub const PFMERGE: &str = "pfmerge";
    pub const PFDEBUG: &str = "pfdebug";
    pub const LATENCY: &str = "latency";
    pub const JGWT_CHECKPOINT: &str = "jgwt-checkpoint";
    pub const JGWT_UNDO: &str = "jgwt-undo";
    pub const JGWT_REDO: &str = "jgwt-redo";
}
...
fn command_properties(command_name: &str) -> CommandProperties {
    ...
    let (arity, flags, first_key_index, last_key_index, key_step) = match command_name {
        command_names::GET => (2, fr, 1, 1, 1),
        command_names::SET => (3, wm, 1, 1, 1),
        command_names::SETNX => (3, wmf, 1, 1, 1),
        command_names::SETEX => (4, wm, 1, 1, 1),
        command_names::PSETEX => (4, wm, 1, 1, 1),
        command_names::APPEND => (3, wm, 1, 1, 1),
        command_names::STRLEN => (2, fr, 1, 1, 1),
        command_names::DEL => (-2, WRITE, 1, -1, 1),
        command_names::EXISTS => (-2, fr, 1, -1, 1),
        command_names::SETBIT => (4, wm, 1, 1, 1),
        command_names::GETBIT => (3, fr, 1, 1, 1),
        command_names::SETRANGE => (4, wm, 1, 1, 1),
        command_names::GETRANGE => (4, READONLY, 1, 1, 1),
        command_names::SUBSTR => (4, READONLY, 1, 1, 1),
        command_names::INCR => (2, wmf, 1, 1, 1),
        command_names::DECR => (2, wmf, 1, 1, 1),
        command_names::MGET => (-2, READONLY, 1, -1, 1),
        command_names::RPUSH => (-3, wmf, 1, 1, 1),
        command_names::LPUSH => (-3, wmf, 1, 1, 1),
        command_names::RPUSHX => (-3, wmf, 1, 1, 1),
        command_names::LPUSHX => (-3, wmf, 1, 1, 1),
        command_names::LINSERT => (5, wm, 1, 1, 1),
        command_names::RPOP => (2, wf, 1, 1, 1),
        command_names::LPOP => (2, wf, 1, 1, 1),
        command_names::RPOPLPUSH => (3, wm, 1, 2, 1),
        command_names::BRPOP => (-3, WRITE | NOSCRIPT, 1, -2, 1),
        command_names::BLPOP => (-3, WRITE | NOSCRIPT, 1, -2, 1),
        command_names::BRPOPLPUSH => (4, wm | NOSCRIPT, 1, 2, 1),
        command_names::LLEN => (2, fr, 1, 1, 1),
        command_names::LINDEX => (3, READONLY, 1, 1, 1),
        command_names::LSET => (4, wm, 1, 1, 1),
        command_names::LRANGE => (4, READONLY, 1, 1, 1),
        command_names::LTRIM => (4, READONLY, 1, 1, 1),
        command_names::LREM => (4, READONLY, 1, 1, 1),
        command_names::SADD => (-3, wmf, 1, 1, 1),
        command_names::SREM => (-3, wf, 1, 1, 1),
        command_names::SMOVE => (4, wf, 1, 2, 1),
        command_names::SISMEMBER => (3, fr, 1, 1, 1),
        command_names::SCARD => (2, fr, 1, 1, 1),
        command_names::SPOP => (-2, fr | RANDOM | NOSCRIPT, 1, 1, 1),
        command_names::SRANDMEMBER => (-2, READONLY | RANDOM, 1, 1, 1),
        command_names::SINTER => (-2, sr, 1, -1, 1),
        command_names::SINTERSTORE => (-3, wm, 1, -1, 1),
        command_names::SUNION => (-2, sr, 1, -1, 1),
        command_names::SUNIONSTORE => (-3, wm, 1, -1, 1),
        command_names::SDIFF => (-2, sr, 1, -1, 1),
        command_names::SDIFFSTORE => (-3, wm, 1, -1, 1),
        command_names::SMEMBERS => (2, sr, 1, 1, 1),
        command_names::SSCAN => (-3, READONLY | RANDOM, 1, 1, 1),
        command_names::ZADD => (-4, wmf, 1, 1, 1),
        command_names::ZINCRBY => (4, wmf, 1, 1, 1),
        command_names::ZREM => (-3, wf, 1, 1, 1),
        command_names::ZREMRANGEBYSCORE => (4, WRITE, 1, 1, 1),
        command_names::ZREMRANGEBYRANK => (4, WRITE, 1, 1, 1),
        command_names::ZREMRANGEBYLEX => (4, WRITE, 1, 1, 1),
        command_names::ZUNIONSTORE => (-4, wm, 0, 0, 0),
        command_names::ZINTERSTORE => (-4, wm, 0, 0, 0),
        command_names::ZRANGE => (-4, READONLY, 1, 1, 1),
        command_names::ZREVRANGE => (-4, READONLY, 1, 1, 1),
        command_names::ZRANGEBYSCORE => (-4, READONLY, 1, 1, 1),
        command_names::ZREVRANGEBYSCORE => (-4, READONLY, 1, 1, 1),
        command_names::ZRANGEBYLEX => (-4, READONLY, 1, 1, 1),
        command_names::ZREVRANGEBYLEX => (-4, READONLY, 1, 1, 1),
        command_names::ZCOUNT => (4, fr, 1, 1, 1),
        command_names::ZLEXCOUNT => (4, fr, 1, 1, 1),
        command_names::ZCARD => (2, fr, 1, 1, 1),
        command_names::ZSCORE => (3, fr, 1, 1, 1),
        command_names::ZRANK => (3, fr, 1, 1, 1),
        command_names::ZREVRANK => (3, fr, 1, 1, 1),
        command_names::ZSCAN => (-3, READONLY | RANDOM, 1, 1, 1),
        command_names::HSET => (4, wmf, 1, 1, 1),
        command_names::HSETNX => (4, wmf, 1, 1, 1),
        command_names::HGET => (3, fr, 1, 1, 1),
        command_names::HMSET => (-4, wm, 1, 1, 1),
        command_names::HMGET => (-3, READONLY, 1, 1, 1),
        command_names::HINCRBY => (4, wmf, 1, 1, 1),
        command_names::HINCRBYFLOAT => (4, wmf, 1, 1, 1),
        command_names::HDEL => (-3, wf, 1, 1, 1),
        command_names::HLEN => (2, fr, 1, 1, 1),
        command_names::HSTRLEN => (3, fr, 1, 1, 1),
        command_names::HKEYS => (2, sr, 1, 1, 1),
        command_names::HVALS => (2, sr, 1, 1, 1),
        command_names::HGETALL => (2, READONLY, 1, 1, 1),
        command_names::HEXISTS => (3, fr, 1, 1, 1),
        command_names::HSCAN => (-3, READONLY | RANDOM, 1, 1, 1),
        command_names::INCRBY => (3, wmf, 1, 1, 1),
        command_names::DECRBY => (3, wmf, 1, 1, 1),
        command_names::INCRBYFLOAT => (3, wmf, 1, 1, 1),
        command_names::GETSET => (3, wm, 1, 1, 1),
        command_names::MSET => (-3, wm, 1, -1, 2),
        command_names::MSETNX => (-3, wm, 1, -1, 2),
        command_names::RANDOMKEY => (1, READONLY | RANDOM, 0, 0, 0),
        command_names::SELECT => (2, fr | LOADING, 0, 0, 0),
        command_names::MOVE => (3, wf, 1, 1, 1),
        command_names::RENAME => (3, WRITE, 1, 2, 1),
        command_names::RENAMENX => (3, wf, 1, 2, 1),
        command_names::EXPIRE => (3, wf, 1, 1, 1),
        command_names::EXPIREAT => (3, wf, 1, 1, 1),
        command_names::PEXPIRE => (3, wf, 1, 1, 1),
        command_names::PEXPIREAT => (3, wf, 1, 1, 1),
        command_names::KEYS => (2, sr, 0, 0, 0),
        command_names::SCAN => (-2, READONLY | RANDOM, 0, 0, 0),
        command_names::DBSIZE => (1, fr, 0, 0, 0),
        command_names::AUTH => (2, fr | NOSCRIPT | ls, 0, 0, 0),
        command_names::PING => (-1, fr | STALE, 0, 0, 0),
        command_names::ECHO => (2, fr, 0, 0, 0),
        command_names::SAVE => (1, ars, 0, 0, 0),
        command_names::BGSAVE => (1, READONLY | ADMIN, 0, 0, 0),
        command_names::BGREWRITEAOF => (1, READONLY | ADMIN, 0, 0, 0),
        command_names::SHUTDOWN => (-1, READONLY | ADMIN | ls, 0, 0, 0),
        command_names::LASTSAVE => (1, fr | RANDOM, 0, 0, 0),
        command_names::TYPE => (2, fr, 1, 1, 1),
        command_names::MULTI => (1, fr | NOSCRIPT, 0, 0, 0),
        command_names::EXEC => (1, NOSCRIPT | SKIP_MONITOR, 0, 0, 0),
        command_names::DISCARD => (1, fr | NOSCRIPT, 0, 0, 0),
        command_names::SYNC => (1, ars, 0, 0, 0),
        command_names::PSYNC => (3, ars, 0, 0, 0),
        command_names::REPLCONF => (-1, ars | ls, 0, 0, 0),
        command_names::FLUSHDB => (1, WRITE, 0, 0, 0),
        command_names::FLUSHALL => (1, WRITE, 0, 0, 0),
        command_names::SORT => (-2, wm, 1, 1, 1),
        command_names::INFO => (-1, READONLY | ls, 0, 0, 0),
        command_names::MONITOR => (1, ars, 0, 0, 0),
        command_names::TTL => (2, fr, 1, 1, 1),
        command_names::PTTL => (2, fr, 1, 1, 1),
        command_names::PERSIST => (2, wf, 1, 1, 1),
        command_names::SLAVEOF => (3, ADMIN | NOSCRIPT | STALE, 0, 0, 0),
        command_names::ROLE => (1, STALE | LOADING | NOSCRIPT, 0, 0, 0),
        command_names::DEBUG => (-2, ADMIN | NOSCRIPT, 0, 0, 0),
        command_names::CONFIG => (-2, ADMIN | READONLY | STALE, 0, 0, 0),
        command_names::SUBSCRIBE => (-2, READONLY | PUBSUB | NOSCRIPT | LOADING | STALE, 0, 0, 0),
        command_names::UNSUBSCRIBE => (-1, READONLY | PUBSUB | NOSCRIPT | LOADING | STALE, 0, 0, 0),
        command_names::PSUBSCRIBE => (-2, READONLY | PUBSUB | NOSCRIPT | LOADING | STALE, 0, 0, 0),
        command_names::PUNSUBSCRIBE => {
            (-1, READONLY | PUBSUB | NOSCRIPT | LOADING | STALE, 0, 0, 0)
        }
        command_names::PUBLISH => (-1, READONLY | PUBSUB | LOADING | STALE | FAST, 0, 0, 0),
        command_names::PUBSUB => (-1, READONLY | PUBSUB | LOADING | STALE | RANDOM, 0, 0, 0),
        ...
        command_names::RESTORE_ASKING => (-4, wm | ASKING, 1, 1, 1),
        command_names::MIGRATE => (-6, WRITE, 0, 0, 0),
        command_names::ASKING => (1, READONLY, 0, 0, 0),
        command_names::READONLY => (1, fr, 0, 0, 0),
        command_names::READWRITE => (1, fr, 0, 0, 0),
        command_names::DUMP => (2, READONLY, 1, 1, 1),
        command_names::OBJECT => (3, READONLY, 2, 2, 2),
        command_names::CLIENT => (-2, READONLY | NOSCRIPT, 0, 0, 0),
        command_names::EVAL => (-3, NOSCRIPT, 0, 0, 0),
        command_names::EVALSHA => (-3, NOSCRIPT, 0, 0, 0),
        command_names::SLOWLOG => (-2, READONLY, 0, 0, 0),
        command_names::SCRIPT => (-2, READONLY | NOSCRIPT, 0, 0, 0),
        command_names::TIME => (1, READONLY | RANDOM | FAST, 0, 0, 0),
        command_names::BITOP => (-4, wm, 2, -1, 1),
        command_names::BITCOUNT => (-2, READONLY, 1, 1, 1),
        command_names::BITPOS => (-3, READONLY, 1, 1, 1),
        command_names::WAIT => (3, READONLY | NOSCRIPT, 0, 0, 0),
        command_names::COMMAND => (0, READONLY | LOADING | STALE, 0, 0, 0),
        command_names::GEOADD => (-5, wm, 1, 1, 1),
        command_names::GEORADIUS => (-6, READONLY, 1, 1, 1),
        command_names::GEORADIUSBYMEMBER => (-5, READONLY, 1, 1, 1),
        command_names::GEOHASH => (-2, READONLY, 1, 1, 1),
        command_names::GEOPOS => (-2, READONLY, 1, 1, 1),
        command_names::GEODIST => (-4, READONLY, 1, 1, 1),
        command_names::PFSELFTEST => (1, READONLY, 1, 1, 1),
        command_names::PFADD => (-2, wmf, 1, 1, 1),
        command_names::PFCOUNT => (-2, READONLY, 1, -1, 1),
        command_names::PFMERGE => (-2, wm, 1, -1, 1),
        command_names::PFDEBUG => (-3, WRITE, 0, 0, 0),
        command_names::LATENCY => (-2, ars | ls, 0, 0, 0),
        command_names::JGWT_CHECKPOINT => (1, wmf, 0, 0, 0),
        command_names::JGWT_UNDO => (1, wmf, 0, 0, 0),
        command_names::JGWT_REDO => (1, wmf, 0, 0, 0),
...
fn execute_command(
    ...
    *write = !command_properties(command_name)
    ...
    Ok(match command_name {
        command_names::PEXPIREAT => pexpireat(parser, db, dbindex),
        command_names::PEXPIRE => pexpire(parser, db, dbindex),
        command_names::EXPIREAT => expireat(parser, db, dbindex),
        command_names::EXPIRE => expire(parser, db, dbindex),
        command_names::TTL => ttl(parser, db, dbindex),
        command_names::PTTL => pttl(parser, db, dbindex),
        command_names::PERSIST => persist(parser, db, dbindex),
        command_names::TYPE => dbtype(parser, db, dbindex),
        command_names::SET => set(parser, db, dbindex),
        command_names::SETNX => setnx(parser, db, dbindex),
        command_names::SETEX => setex(parser, db, dbindex),
        command_names::PSETEX => psetex(parser, db, dbindex),
        command_names::DEBUG => debug(parser, db, dbindex),
        command_names::DEL => del(parser, db, dbindex),
        command_names::DBSIZE => dbsize(parser, db, dbindex),
        command_names::APPEND => append(parser, db, dbindex),
        command_names::GET => get(parser, db, dbindex),
        command_names::GETRANGE => getrange(parser, db, dbindex),
        command_names::MGET => mget(parser, db, dbindex),
        command_names::SUBSTR => getrange(parser, db, dbindex),
        command_names::SETRANGE => setrange(parser, db, dbindex),
        command_names::SETBIT => setbit(parser, db, dbindex),
        command_names::GETBIT => getbit(parser, db, dbindex),
        command_names::STRLEN => strlen(parser, db, dbindex),
        command_names::INCR => incr(parser, db, dbindex),
        command_names::DECR => decr(parser, db, dbindex),
        command_names::INCRBY => incrby(parser, db, dbindex),
        command_names::DECRBY => decrby(parser, db, dbindex),
        command_names::INCRBYFLOAT => incrbyfloat(parser, db, dbindex),
        command_names::PFADD => pfadd(parser, db, dbindex),
        command_names::PFCOUNT => pfcount(parser, db, dbindex),
        command_names::PFMERGE => pfmerge(parser, db, dbindex),
        command_names::EXISTS => exists(parser, db, dbindex),
        command_names::PING => ping(parser, client),
        command_names::FLUSHDB => flushdb(parser, db, dbindex),
        command_names::FLUSHALL => flushall(parser, db, dbindex),
        command_names::LPUSH => lpush(parser, db, dbindex),
        command_names::RPUSH => rpush(parser, db, dbindex),
        command_names::LPUSHX => lpushx(parser, db, dbindex),
        command_names::RPUSHX => rpushx(parser, db, dbindex),
        command_names::LPOP => lpop(parser, db, dbindex),
        command_names::RPOP => rpop(parser, db, dbindex),
        command_names::LINDEX => lindex(parser, db, dbindex),
        command_names::LINSERT => linsert(parser, db, dbindex),
        command_names::LLEN => llen(parser, db, dbindex),
        command_names::LRANGE => lrange(parser, db, dbindex),
        command_names::LREM => lrem(parser, db, dbindex),
        command_names::LSET => lset(parser, db, dbindex),
        command_names::LTRIM => ltrim(parser, db, dbindex),
        command_names::RPOPLPUSH => rpoplpush(parser, db, dbindex),
        command_names::BRPOPLPUSH => brpoplpush(parser, db, dbindex)?,
        command_names::BRPOP => brpop(parser, db, dbindex)?,
        command_names::BLPOP => blpop(parser, db, dbindex)?,
        command_names::SADD => sadd(parser, db, dbindex),
        command_names::SREM => srem(parser, db, dbindex),
        command_names::SISMEMBER => sismember(parser, db, dbindex),
        command_names::SMEMBERS => smembers(parser, db, dbindex),
        command_names::SRANDMEMBER => srandmember(parser, db, dbindex),
        command_names::SPOP => spop(parser, db, dbindex),
        command_names::SMOVE => smove(parser, db, dbindex),
        command_names::SCARD => scard(parser, db, dbindex),
        command_names::SDIFF => sdiff(parser, db, dbindex),
        command_names::SDIFFSTORE => sdiffstore(parser, db, dbindex),
        command_names::SINTER => sinter(parser, db, dbindex),
        command_names::SINTERSTORE => sinterstore(parser, db, dbindex),
        command_names::SUNION => sunion(parser, db, dbindex),
        command_names::SUNIONSTORE => sunionstore(parser, db, dbindex),
        command_names::ZADD => zadd(parser, db, dbindex),
        command_names::ZCARD => zcard(parser, db, dbindex),
        command_names::ZSCORE => zscore(parser, db, dbindex),
        command_names::ZINCRBY => zincrby(parser, db, dbindex),
        command_names::ZREM => zrem(parser, db, dbindex),
        command_names::ZREMRANGEBYLEX => zremrangebylex(parser, db, dbindex),
        command_names::ZREMRANGEBYSCORE => zremrangebyscore(parser, db, dbindex),
        command_names::ZREMRANGEBYRANK => zremrangebyrank(parser, db, dbindex),
        command_names::ZCOUNT => zcount(parser, db, dbindex),
        command_names::ZLEXCOUNT => zlexcount(parser, db, dbindex),
        command_names::ZRANGE => zrange(parser, db, dbindex),
        command_names::ZREVRANGE => zrevrange(parser, db, dbindex),
        command_names::ZRANGEBYSCORE => zrangebyscore(parser, db, dbindex),
        command_names::ZREVRANGEBYSCORE => zrevrangebyscore(parser, db, dbindex),
        command_names::ZRANGEBYLEX => zrangebylex(parser, db, dbindex),
        command_names::ZREVRANGEBYLEX => zrevrangebylex(parser, db, dbindex),
        command_names::ZRANK => zrank(parser, db, dbindex),
        command_names::ZREVRANK => zrevrank(parser, db, dbindex),
        command_names::ZUNIONSTORE => zunionstore(parser, db, dbindex),
        command_names::ZINTERSTORE => zinterstore(parser, db, dbindex),
        command_names::DUMP => dump(parser, db, dbindex),
        command_names::KEYS => keys(parser, db, dbindex),
        command_names::WATCH => watch(parser, db, dbindex, client.id, &mut client.watched_keys),
        command_names::UNWATCH => unwatch(parser, db, client.id, &mut client.watched_keys),
        command_names::SUBSCRIBE => subscribe(
            parser,
            db,
        ...
        command_names::UNSUBSCRIBE => unsubscribe(
            parser,
            db,
            &mut client.subscriptions,
            client.pattern_subscriptions.len(),
        ...
        command_names::PSUBSCRIBE => psubscribe(
            parser,
            db,
        ...
        command_names::PUNSUBSCRIBE => punsubscribe(
            parser,
            db,
            client.subscriptions.len(),
            &mut client.pattern_subscriptions,
        ...
        command_names::PUBLISH => publish(parser, db),
        command_names::MONITOR => {
            *log = false;
            monitor(parser, db, client.rawsender.clone())
        }
        command_names::INFO => info(parser, db),
        command_names::JGWT_CHECKPOINT => unimplemented!(),
        command_names::JGWT_UNDO => unimplemented!(),
        command_names::JGWT_REDO => unimplemented!(),
